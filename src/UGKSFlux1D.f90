...
subroutine flux_ugks1d(wL, hL, bL, shL, sbL, lenL, fluxw, fluxh, fluxb, wR, hR, bR, shR, sbR, lenR, unum, uspace, weight, ink, gamma, muref, omega, prandtl, dt)

real(kind=8), dimension(:), intent(in) :: wL, wR !// conservative variables
real(kind=8), dimension(:), intent(in) :: hL, bL, shL, sbL, hR, bR, shR, sbR !// distribution functions and their slopes
real(kind=8), dimension(:), intent(inout) :: fluxw, fluxh, fluxb !// interface fluxes
real(kind=8), intent(in) :: lenL, lenR !// cell lengths
integer, intent(in) :: unum !// number of velocity grids
real(kind=8), dimension(:), intent(in) :: uspace, weight !// velocity quadrature points and weights
...
fluxw(1) = fluxw(1) + Mt(1) * sum(weight * uspace * H_plus) + Mt(4) * sum(weight * uspace * h) - Mt(5) * sum(weight * uspace**2 * sh)
fluxw(2) = fluxw(2) + Mt(1) * sum(weight * uspace**2 * H_plus) + Mt(4) * sum(weight * uspace**2 * h) - Mt(5) * sum(weight * uspace**3 * sh)
fluxw(3) = fluxw(3) +&
...
function micro_slope(prim, sw, ink)
    ...
    micro_slope(3) = 4.d0 * prim(3)**2 / (ink + 1.d0) / prim(1) * (2.d0 * sw(3) - 2.d0 * prim(2) * sw(2) + sw(1) * (prim(2)**2 - 0.5d0 * (ink + 1.d0) / prim(3)))
    micro_slope(2) = 2.d0 * prim(3) / prim(1) * (sw(2) - prim(2) * sw(1)) - prim(2) * micro_slope(3)
    micro_slope(1) = sw(1) / prim(1) - prim(2) * micro_slope(2) - 0.5d0 * (prim(2)**2 + 0.5d0 * (ink + 1.d0) / prim(3)) * micro_slope(3)
